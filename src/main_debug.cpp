...
static LRESULT CALLBACK WndProc(HWND h, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_LBUTTONDOWN: {
        POINT p{ GET_X_LPARAM(lp), GET_Y_LPARAM(lp) };
        dbg("WM_LBUTTONDOWN: x=%d y=%d HH=%d hitCard=%d\n", p.x, p.y, HH, hitCard(p));
    ...
    case WM_LBUTTONUP: {
        POINT p{ GET_X_LPARAM(lp), GET_Y_LPARAM(lp) };
        dbg("WM_LBUTTONUP: x=%d y=%d hitCard=%d\n", p.x, p.y, hitCard(p));
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int)
{
    ...
    // Log button layout
    for (auto& b : g_btns) {
        dbg("Btn id=%d rect=(%d,%d)-(%d,%d) title=[%ls]\n",
            b.id, b.r.left, b.r.top, b.r.right, b.r.bottom, b.title.c_str());
    }
